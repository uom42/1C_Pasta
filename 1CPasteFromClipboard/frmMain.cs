...
!!!Вставляемое значение в справочнике 1С уже должно существовать, иначе встравка не получится и будут глюки!";
			...
			//labelClickHandler1.OpenUrlError += (s, ex) => MessageBox.Show(ex.Message, Application.ProductName, MessageBoxButtons.OK, MessageBoxIcon.Error);

			ReadClipboard();

			//'Make a Background Thread that handles Background Tasks
			Thread BackgroundThread = new(BackgroundTasks)
			{
				IsBackground = true
			};
