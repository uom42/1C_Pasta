...
return (SourceValue & BitMask) == BitMask;
	 ...
	 internal static int e_SetBitByMaskWithOffset(this int SourceValue, int iZeroBasedStartBitIndex, int iBitCountAtValue, int iValueToSet)
	 {
		...
		internal static class Extensions_StringAndFormat
		{
			...
			/// <param name="src">Массив строк для объединения</param>
				...
				)
			{
				if (enquoteAllStrings)
					src = src.Select(s => ('"' + s + '"')).ToArray();

				return ((null == src) || !src!.Any())
					? EmptyOrNothingValue
	...
	# End Region
		...
		internal static partial class Extensions_IO
		{
			...
			[MethodImpl(MethodImplOptions.AggressiveInlining)]
			internal static string e_ReadToEnd(this FileInfo f, System.Text.Encoding? enc = null, bool detectEncodingFromByteOrderMarks = false)
			{
			...
			/// <summary>Read to RAM</summary>
			public static MemoryStream e_ReadToMemory(this FileInfo fi)
			{
				using (FileStream fsSource = new(fi.FullName, FileMode.Open))
				{
					MemoryStream msImage = new();
					fsSource.CopyTo(msImage);
...
#pragma warning restore SYSLIB0014 // Type or member is obsolete
	...
	#region Old
		...
		internal static class Extensions_XML
		{
			...
			[DebuggerNonUserCode, DebuggerStepThrough, MethodImpl(MethodImplOptions.AggressiveInlining)]
			public static XmlNode[] e_ToArray(this XmlNodeList xnl)
				=> xnl.Cast<XmlNode>().ToArray();
