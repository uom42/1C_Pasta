...
3. Для ввода ячейки имитируется "набор посимвольно с клавиатуры", потом имитируется нажатие ***TAB*** для переключения на следующую ячейку.

    Если отмечена галка  "***Первая колонка это справочник***", считается что эта колонка - это выбор из справочника.
    
    **ВАЖНО!!! В справочнике уже должен ранее существовать такой элемент**, мы не создаём новые элементы справочника!**
    
    Значение для вставки в ячейку, в таком случае, не "набирается посимвольно с клавиатуры", а сначала копируется в системный буфер обмена ОС, потом в окне 1С имитируется нажатие ***Ctrl+V***, затем ***ENTER*** для подтверждения выбора элемента из справочника.

7. После ввода всех ячеек строки, имитируется нажатие ***F3***.


### Некоторые особенности и ограничения:
* Это работает только для "простых" однострочных строк в таблице.
